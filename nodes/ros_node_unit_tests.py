...
class UnitTestsMPC(object):
    ...
    def get_setpoint(self, t):
        ...
        if self.test_num == 1:
            ...
            if t < self.test_time:
                ...
                xd = np.array([self.test_targets['t']['t7']]).reshape((self.Nx, 1))
            else:
                xd = np.array([self.test_targets['t']['t7']]).reshape((self.Nx, 1))
            ...
            if t < self.test_time:
                ...
                xd = np.array([self.test_targets['q']['t7']]).reshape((self.Nx, 1))
            else:
                xd = np.array([self.test_targets['q']['t7']]).reshape((self.Nx, 1))
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if t > CONTROL_HANDOVER_DELAY and t < self.expiration_time and self.obc_state is True:
